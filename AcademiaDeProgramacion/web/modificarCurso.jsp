...
<html>
    ...
    <body>
        ...
        <br>
        <form method="POST" action="/AcademiaDeProgramacion/ModificarCurso"> 
            <input type="hidden" name="txtIdCurso" value="${cursodto.curso.idCurso}" /> 
            <div class="container">
                <div class="form-group col-md-6">
                    <label for="txtNombreCurso">Nombre del Curso: </label> 
                    <input type="text" name="txtNombreCurso" id="txtNombreCurso" class="form-control" value="${cursodto.curso.nombreCurso}"/>
                </div>

                <div class="form-group col-md-6">
                    <label for="cboDocente">Docente: </label>
                    <select name="cboDocente"  class="form-control">
                        <c:forEach var="docente" items="${cursodto.listaDocente}">
                            <option value="${docente.idDocente}" <c:if test="${docente.idDocente == cursodto.curso.docente.idDocente}">so. selected</c:if>>${docente.nombre} ${docente.apellido}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="form-group col-md-6">
                    <label for="txtDuracion">Duracion: </label>
                    <input type="text" name="txtDuracion" class="form-control" value="${cursodto.curso.duracion}"/>
                </div>

                <div class="form-group col-md-6">
                    <label for="txtModalidad">Modalidad: </label>
                    <input type="text" name="txtModalidad" class="form-control" value="${cursodto.curso.modalidad}"/>
                </div>           
                <div class="form-group col-md-6">
                    <label for="txtPrecio">Precio: </label>
                    <input type="number" name="txtPrecio" class="form-control" value="${cursodto.curso.precio}"/>
                </div>  
